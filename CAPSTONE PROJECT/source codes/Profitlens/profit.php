...
// Soft-delete: move a sale to Deleted History instead of removing it outright
if ($action === 'delete' && isset($_GET['id'])) {
    $id = intval($_GET['id']);
    $db->query("UPDATE sales SET deleted_at = NOW() WHERE id=$id");
    $success = 'Sale moved to Deleted History.';
}

// Bulk soft-delete of entire sales history (admin only, requires confirmation on the client)
if ($action === 'delete_all') {
    $db->query("UPDATE sales SET deleted_at = NOW() WHERE deleted_at IS NULL");
    $success = 'All sales history has been moved to Deleted History.';
}

// Restore a single sale out of Deleted History
if ($action === 'restore' && isset($_GET['id'])) {
    $id = intval($_GET['id']);
    $db->query("UPDATE sales SET deleted_at = NULL WHERE id=$id");
    $success = 'Sale restored.';
}

// Restore everything currently in Deleted History
if ($action === 'restore_all') {
    $db->query("UPDATE sales SET deleted_at = NULL WHERE deleted_at IS NOT NULL");
    $success = 'All deleted sales have been restored.';
}

// Permanently remove a single sale from Deleted History (irreversible)
if ($action === 'purge' && isset($_GET['id'])) {
    $id = intval($_GET['id']);
    $db->query("DELETE FROM sales WHERE id=$id AND deleted_at IS NOT NULL");
    $success = 'Sale permanently deleted.';
}

// Empty the entire Deleted History (irreversible)
if ($action === 'purge_all') {
    $db->query("DELETE FROM sales WHERE deleted_at IS NOT NULL");
    $success = 'Deleted History has been permanently emptied.';
}
...
// Use YEAR() + MONTH() for reliable matching
$cur  = $db->query("SELECT SUM(total) as sales, COUNT(*) as count FROM sales WHERE deleted_at IS NULL AND YEAR(sale_date)=$cur_year AND MONTH(sale_date)=$cur_month")->fetch_assoc();
...
$last = $db->query("SELECT SUM(total) as sales FROM sales WHERE deleted_at IS NULL AND YEAR(sale_date)=$last_year AND MONTH(sale_date)=$last_mon")->fetch_assoc();
...
for ($m = 1; $m <= 12; $m++) {
    ...
    $row = $db->query("SELECT COALESCE(SUM(total),0) as t FROM sales WHERE deleted_at IS NULL AND DATE_FORMAT(sale_date,'%Y-%m')='$ym'")->fetch_assoc();
    $chart_months[] = floatval($row['t']);
}
$chart_max = max($chart_months) ?: 1;

// Available years for selector
$years_res = $db->query("SELECT DISTINCT YEAR(sale_date) as y FROM sales WHERE deleted_at IS NULL ORDER BY y DESC");
$available_years = [];
...
// Sales history list (active only — soft-deleted rows live in Deleted History)
$sales    = $db->query("SELECT s.*, p.name as product_name FROM sales s LEFT JOIN products p ON s.product_id=p.id WHERE s.deleted_at IS NULL ORDER BY s.sale_date DESC, s.id DESC LIMIT 500");
...
// Deleted History list (soft-deleted sales, most recently deleted first)
$deleted_sales = $db->query("SELECT s.*, p.name as product_name FROM sales s LEFT JOIN products p ON s.product_id=p.id WHERE s.deleted_at IS NOT NULL ORDER BY s.deleted_at DESC, s.id DESC LIMIT 500");
$deleted_sales_arr = [];
if ($deleted_sales) while($r = $deleted_sales->fetch_assoc()) $deleted_sales_arr[] = $r;

// Year breakdown for the Deleted History modal's year pills
$deleted_year_counts = [];
foreach ($deleted_sales_arr as $ds) {
    $dy = date('Y', strtotime($ds['sale_date']));
    $deleted_year_counts[$dy] = ($deleted_year_counts[$dy] ?? 0) + 1;
}
$deleted_years = array_keys($deleted_year_counts);
rsort($deleted_years);
...
<head>
    ...
    <style>
        ...
        .btn-delete-history {
            ...
            padding:7px 14px; background:#dc3545; color:#fff; border:none;
            border-radius:8px; font-size:11px; font-weight:700; cursor:pointer;
            font-family:'Poppins',sans-serif; transition:background .15s;
        }
        .btn-delete-history:hover { background:#b02a37; }
        .btn-delete-history:disabled { background:#e9a8ae; cursor:not-allowed; }

        /* View Deleted History button */
        .btn-view-trash {
            display:inline-flex; align-items:center; gap:6px;
            padding:7px 14px; background:#6c757d; color:#fff; border:none;
            border-radius:8px; font-size:11px; font-weight:700; cursor:pointer;
            font-family:'Poppins',sans-serif; transition:background .15s;
        }
        .btn-view-trash:hover { background:#565e64; }
        .btn-view-trash.active { background:var(--green-main); }
        .trash-count-badge {
            background:rgba(255,255,255,.25); border-radius:10px; padding:1px 7px; font-size:10px;
        }

        /* Deleted History rows */
        .trash-row td { color:#777; }
        .trash-row .product-name { text-decoration:line-through; text-decoration-color:#c9c9c9; }
        .btn-restore, .btn-purge {
            padding:5px 10px; border-radius:6px; border:none; font-size:11px; font-weight:700;
            cursor:pointer; font-family:'Poppins',sans-serif; margin-right:4px;
        }
        .btn-restore { background:#d4edda; color:#155724; }
        .btn-restore:hover { background:#b7dfc1; }
        .btn-purge { background:#f8d7da; color:#721c24; }
        .btn-purge:hover { background:#f1b0b7; }
        .btn-restore-all, .btn-empty-trash {
            display:inline-flex; align-items:center; gap:6px;
            padding:7px 14px; border:none; border-radius:8px; font-size:11px; font-weight:700;
            cursor:pointer; font-family:'Poppins',sans-serif; transition:background .15s;
        }
        .btn-restore-all { background:#28a745; color:#fff; }
        .btn-restore-all:hover { background:#218838; }
        .btn-restore-all:disabled { background:#a9d9b6; cursor:not-allowed; }
        .btn-empty-trash { background:#dc3545; color:#fff; }
        .btn-empty-trash:hover { background:#b02a37; }
        .btn-empty-trash:disabled { background:#e9a8ae; cursor:not-allowed; }

        /* Deleted Sales History modal */
        .deleted-history-modal {
            background:#fff; border-radius:16px; padding:32px 32px 26px; width:100%;
            max-width:820px; max-height:85vh; box-shadow:0 20px 60px rgba(0,0,0,.18);
            text-align:left; animation:modalPop .22s cubic-bezier(0.34,1.56,0.64,1) both;
            display:flex; flex-direction:column;
        }
        .deleted-history-modal h3 {
            font-size:19px; font-weight:700; color:#1a1a1a; margin-bottom:6px;
            font-family:'Poppins',sans-serif; text-align:center;
        }
        .dh-subtitle { font-size:12.5px; color:#777; text-align:center; margin-bottom:18px; font-family:'Poppins',sans-serif; }
        .dh-pills { display:flex; gap:8px; flex-wrap:wrap; margin-bottom:14px; }
        .dh-pill {
            display:inline-flex; align-items:center; gap:6px; padding:6px 14px; border-radius:20px;
            font-size:12px; font-weight:700; cursor:pointer; border:1.5px solid var(--gray-mid,#ddd);
            background:#fff; color:#666; font-family:'Poppins',sans-serif; transition:all .15s;
        }
        .dh-pill.on { background:var(--green-main,#27ae60); color:#fff; border-color:var(--green-main,#27ae60); }
        .dh-pill-count { background:rgba(0,0,0,.08); border-radius:10px; padding:1px 7px; font-size:10px; }
        .dh-pill.on .dh-pill-count { background:rgba(255,255,255,.25); }
        .dh-total-row {
            display:flex; justify-content:space-between; align-items:center;
            background:#f8f9fa; border-radius:8px; padding:10px 14px; margin-bottom:14px;
            font-size:12.5px; font-family:'Poppins',sans-serif; color:#555;
        }
        .dh-total-row strong { color:#dc3545; font-size:14px; }
        .dh-table-wrap { overflow-y:auto; overflow-x:auto; border:1px solid #eee; border-radius:8px; flex:1; }
        .dh-table-wrap table th {
            position:sticky; top:0; z-index:1; background:#f8f9fa; text-align:left;
            font-size:11px; text-transform:uppercase; letter-spacing:.3px; color:#888;
        }

        /* ── Delete Sale / Delete History Modal ── */
        ...
        .delete-modal {
            ...
            text-align: center;
            animation: modalPop .22s cubic-bezier(0.34,1.56,0.64,1) both;
        }
        @keyframes modalPop {
        ...
        .delete-modal .sale-name {
            font-weight: 700;
            color: #1a1a1a;
        }
        .delete-modal .cannot-undo {
            font-size: 12px;
            color: #999;
            margin-bottom: 24px;
            font-family: 'Poppins', sans-serif;
        }
        .delete-modal .cannot-undo strong {
            color: #dc3545;
        }
        ...
        .delete-modal-actions .btn-confirm-delete:active {
            transform: translateY(0);
        }
        .delete-modal-actions .btn-confirm-delete.disabled {
            background: #e9a8ae;
            pointer-events: none;
        }
        /* Type-to-confirm input for bulk delete */
        .delete-modal .confirm-type-input {
            width: 100%;
        ...
        .delete-modal .confirm-type-input:focus { border-color: #dc3545; }
    </style>
</head>
<body>
    ...
    <div class="main-content">
        ...
        <div class="page-content">
            ...
            <div class="two-col">
                ...
                function openProdList() { document.getElementById('prod-list').classList.add('open'); }
                function closeProdList() { document.getElementById('prod-list').classList.remove('open'); }
                ...
                <div class="table-card-header" style="flex-direction:column;align-items:stretch;gap:10px;">
                    <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:10px;">
                        ...
                        <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;">
                            ...
                            <button type="button" class="btn-delete-history" id="delete-history-btn"
                                onclick="openDeleteAllModal()" <?= empty($sales_arr) ? 'disabled' : '' ?>>
                                🗑️ Delete History
                            </button>
                            <button type="button" class="btn-view-trash" id="view-trash-btn" onclick="openDeletedHistoryModal()">
                                🗄️ Deleted History <span class="trash-count-badge"><?= count($deleted_sales_arr) ?></span>
                            </button>
                        </div>
                    </div>
                    <!-- Month/Year Filter -->
                ...
                <table class="data-table">
                    ...
                    <tbody id="sale-tbody">
                        ...
                        <tr class="sale-row"
                            ...
                            <td>
                                <!-- Replaced confirm() with custom modal trigger -->
                                <button
                                    type="button"
                                    class="btn-danger"
                                    onclick="openDeleteModal(<?= $s['id'] ?>, '<?= htmlspecialchars(addslashes($s['product_name'] ?? 'N/A'), ENT_QUOTES) ?>', '<?= $dispDate ?>')"
                                >Delete</button>
                            </td>
                        </tr>
                        <?php endforeach; else: ?>
...
<!-- ── Delete Sale Modal ── -->
<div class="delete-modal-overlay" id="delete-modal-overlay" onclick="handleOverlayClick(event)">
    <div class="delete-modal">
        <div class="delete-modal-icon">🗑️</div>
        <h3>Delete Sale?</h3>
        <p>You're about to delete</p>
        <p><span class="sale-name" id="modal-sale-name">—</span></p>
        <p id="modal-sale-date" style="font-size:12px;color:#888;margin-bottom:4px;font-family:'Poppins',sans-serif;"></p>
        <p class="cannot-undo">It will move to <strong>Deleted History</strong>, where you can restore it anytime.</p>
        <div class="delete-modal-actions">
            <button class="btn-cancel" onclick="closeDeleteModal()">Cancel</button>
            <a id="modal-confirm-link" href="#" class="btn-confirm-delete">Yes, Delete</a>
        </div>
    </div>
</div>

<!-- ── Delete ALL History Modal ── -->
<div class="delete-modal-overlay" id="delete-all-modal-overlay" onclick="handleAllOverlayClick(event)">
    <div class="delete-modal">
        <div class="delete-modal-icon">🗑️</div>
        <h3>Delete Entire Sales History?</h3>
        <p>You're about to delete</p>
        <p><span class="sale-name" id="modal-all-count">all <?= count($sales_arr) ?> sale record(s)</span></p>
        <p style="font-size:12px;color:#888;margin-bottom:4px;">This will remove every transaction shown below, for all years.</p>
        <p class="cannot-undo">They'll move to <strong>Deleted History</strong>, where they can be restored anytime.</p>
        <p style="font-size:12px;color:#555;margin-bottom:6px;">Type <strong>DELETE</strong> to confirm:</p>
        <input type="text" id="confirm-all-input" class="confirm-type-input" placeholder="Type DELETE" autocomplete="off" oninput="checkAllConfirmInput()">
        <div class="delete-modal-actions">
            <button class="btn-cancel" onclick="closeDeleteAllModal()">Cancel</button>
            <a id="modal-confirm-all-link" href="#" class="btn-confirm-delete disabled">Yes, Delete All</a>
        </div>
    </div>
</div>

<!-- ── Deleted Sales History Modal ── -->
<div class="delete-modal-overlay" id="deleted-history-modal-overlay" onclick="handleDeletedHistoryOverlayClick(event)">
    <div class="deleted-history-modal">
        <h3>🗑️ Deleted Sales History</h3>
        <p class="dh-subtitle">Sales that have been deleted, organized by year. Restore or permanently delete a record.</p>

        <div class="dh-pills" id="dh-year-pills">
            <button type="button" class="dh-pill on" data-year="all" onclick="filterDeletedByYear(this,'all')">
                All Years <span class="dh-pill-count"><?= count($deleted_sales_arr) ?></span>
            </button>
            <?php foreach ($deleted_years as $dy): ?>
            <button type="button" class="dh-pill" data-year="<?= $dy ?>" onclick="filterDeletedByYear(this,'<?= $dy ?>')">
                <?= $dy ?> <span class="dh-pill-count"><?= $deleted_year_counts[$dy] ?></span>
            </button>
            <?php endforeach; ?>
        </div>

        <div class="dh-total-row">
            <span id="dh-total-label">Total deleted (All Years)</span>
            <strong id="dh-total-amount"><?= formatMoney(array_sum(array_column($deleted_sales_arr,'total'))) ?></strong>
        </div>

        <div style="display:flex;gap:8px;margin-bottom:10px;">
            <button type="button" class="btn-restore-all" onclick="openRestoreAllModal()" <?= empty($deleted_sales_arr) ? 'disabled' : '' ?>>♻️ Restore All</button>
            <button type="button" class="btn-empty-trash" onclick="openPurgeAllModal()" <?= empty($deleted_sales_arr) ? 'disabled' : '' ?>>🔥 Empty Trash</button>
        </div>
        ...
        <table class="data-table">
            ...
            <tbody id="dh-tbody">
                <?php if ($deleted_sales_arr): foreach($deleted_sales_arr as $s):
                    $dispDate  = date('M d, Y', strtotime($s['sale_date']));
                    $delTime   = !empty($s['deleted_at']) ? date('M d, Y h:i A', strtotime($s['deleted_at'])) : '—';
                    $rowYear   = date('Y', strtotime($s['sale_date']));
                ?>
                <tr class="trash-row" data-year="<?= $rowYear ?>" data-total="<?= floatval($s['total']) ?>">
                    <td class="product-name" style="font-weight:600"><?= htmlspecialchars($s['product_name']??'N/A') ?></td>
                    <td><?= $s['quantity'] ?></td>
                    <td style="color:#dc3545;font-weight:700;"><?= formatMoney($s['total']) ?></td>
                    <td><?= $dispDate ?></td>
                    <td style="font-size:11px;white-space:nowrap;"><?= $delTime ?></td>
                    <td style="white-space:nowrap;">
                        <a class="btn-restore" href="profit.php?action=restore&id=<?= $s['id'] ?>&year=<?= $view_year ?>">Restore</a>
                        <button type="button" class="btn-purge"
                            onclick="openPurgeModal(<?= $s['id'] ?>, '<?= htmlspecialchars(addslashes($s['product_name'] ?? 'N/A'), ENT_QUOTES) ?>', '<?= $dispDate ?>')"
                        >Purge</button>
                    </td>
                </tr>
                <?php endforeach; else: ?>
                <tr id="dh-empty-row"><td colspan="6" style="text-align:center;color:var(--gray);padding:30px;">Deleted History is empty</td></tr>
                <?php endif; ?>
            </tbody>
        </table>
        </div>

        <button type="button" class="btn-cancel" style="width:100%;margin-top:18px;" onclick="closeDeletedHistoryModal()">Close</button>
    </div>
</div>

<!-- ── Delete Permanently (single) Modal ── -->
<div class="delete-modal-overlay" id="purge-modal-overlay" onclick="handlePurgeOverlayClick(event)">
    <div class="delete-modal">
        <div class="delete-modal-icon">🔥</div>
        <h3>Delete Permanently?</h3>
        <p>You're about to permanently delete</p>
        <p><span class="sale-name" id="purge-modal-sale-name">—</span></p>
        <p id="purge-modal-sale-date" style="font-size:12px;color:#888;margin-bottom:4px;font-family:'Poppins',sans-serif;"></p>
        <p class="cannot-undo">This action <strong>cannot be undone</strong> — it will be gone for good.</p>
        <div class="delete-modal-actions">
            <button class="btn-cancel" onclick="closePurgeModal()">Cancel</button>
            <a id="purge-modal-confirm-link" href="#" class="btn-confirm-delete">Yes, Delete Forever</a>
        </div>
    </div>
</div>

<!-- ── Empty Trash (purge all) Modal ── -->
<div class="delete-modal-overlay" id="purge-all-modal-overlay" onclick="handlePurgeAllOverlayClick(event)">
    <div class="delete-modal">
        <div class="delete-modal-icon">🔥</div>
        <h3>Empty Trash?</h3>
        <p>You're about to permanently delete</p>
        <p><span class="sale-name" id="purge-all-count">all <?= count($deleted_sales_arr) ?> sale record(s) in Deleted History</span></p>
        <p class="cannot-undo">This action <strong>cannot be undone</strong> — they will be gone for good.</p>
        <p style="font-size:12px;color:#555;margin-bottom:6px;">Type <strong>DELETE</strong> to confirm:</p>
        <input type="text" id="confirm-purge-all-input" class="confirm-type-input" placeholder="Type DELETE" autocomplete="off" oninput="checkPurgeAllConfirmInput()">
        <div class="delete-modal-actions">
            <button class="btn-cancel" onclick="closePurgeAllModal()">Cancel</button>
            <a id="modal-confirm-purge-all-link" href="#" class="btn-confirm-delete disabled">Yes, Empty Trash</a>
        </div>
    </div>
</div>

<!-- ── Restore All Modal ── -->
<div class="delete-modal-overlay" id="restore-all-modal-overlay" onclick="handleRestoreAllOverlayClick(event)">
    <div class="delete-modal">
        <div class="delete-modal-icon">♻️</div>
        <h3>Restore All Sales?</h3>
        <p>You're about to restore</p>
        <p><span class="sale-name" id="restore-all-count">all <?= count($deleted_sales_arr) ?> sale record(s)</span></p>
        <p style="font-size:12px;color:#888;margin-bottom:24px;">They'll reappear in your active Sales History.</p>
        <div class="delete-modal-actions">
            <button class="btn-cancel" onclick="closeRestoreAllModal()">Cancel</button>
            <a id="modal-confirm-restore-all-link" href="#" class="btn-confirm-delete" style="background:#28a745;">Yes, Restore All</a>
        </div>
    </div>
</div>
...
// ── Delete Single Sale Modal ──
function openDeleteModal(id, productName, saleDate) {
    document.getElementById('modal-sale-name').textContent = productName;
    document.getElementById('modal-sale-date').textContent = 'Date: ' + saleDate;
    document.getElementById('modal-confirm-link').href = 'profit.php?action=delete&id=' + id + '&year=<?= $view_year ?>';
...
function openDeleteAllModal() {
    ...
    checkAllConfirmInput();
    document.getElementById('modal-confirm-all-link').href = 'profit.php?action=delete_all&year=<?= $view_year ?>';
    document.getElementById('delete-all-modal-overlay').classList.add('open');
    document.body.style.overflow = 'hidden';
}

function closeDeleteAllModal() {
    document.getElementById('delete-all-modal-overlay').classList.remove('open');
    document.body.style.overflow = '';
}

function handleAllOverlayClick(e) {
    if (e.target === document.getElementById('delete-all-modal-overlay')) {
        closeDeleteAllModal();
    }
}

function checkAllConfirmInput() {
    const val = document.getElementById('confirm-all-input').value.trim().toUpperCase();
    const link = document.getElementById('modal-confirm-all-link');
    if (val === 'DELETE') {
...
function openDeletedHistoryModal() {
    document.getElementById('deleted-history-modal-overlay').classList.add('open');
    document.body.style.overflow = 'hidden';
}
function closeDeletedHistoryModal() {
    document.getElementById('deleted-history-modal-overlay').classList.remove('open');
    document.body.style.overflow = '';
}
function handleDeletedHistoryOverlayClick(e) {
    if (e.target === document.getElementById('deleted-history-modal-overlay')) closeDeletedHistoryModal();
}
function filterDeletedByYear(el, year) {
    document.querySelectorAll('.dh-pill').forEach(p => p.classList.remove('on'));
    el.classList.add('on');
    let total = 0, count = 0;
    document.querySelectorAll('#dh-tbody .trash-row').forEach(row => {
        const show = (year === 'all') || (row.dataset.year === year);
        row.style.display = show ? '' : 'none';
        if (show) { total += parseFloat(row.dataset.total || 0); count++; }
    });
    document.getElementById('dh-total-label').textContent = 'Total deleted (' + (year === 'all' ? 'All Years' : year) + ')';
    document.getElementById('dh-total-amount').textContent = '₱' + total.toLocaleString('en-PH', {minimumFractionDigits:2});
    const emptyRow = document.getElementById('dh-empty-row');
    if (emptyRow) emptyRow.style.display = count === 0 ? '' : 'none';
}

// ── Empty Trash (purge all) Modal ──
function openPurgeAllModal() {
    document.getElementById('confirm-purge-all-input').value = '';
    checkPurgeAllConfirmInput();
    document.getElementById('modal-confirm-purge-all-link').href = 'profit.php?action=purge_all&year=<?= $view_year ?>';
    document.getElementById('purge-all-modal-overlay').classList.add('open');
    document.body.style.overflow = 'hidden';
}
function closePurgeAllModal() {
    document.getElementById('purge-all-modal-overlay').classList.remove('open');
    document.body.style.overflow = '';
}
function handlePurgeAllOverlayClick(e) {
    if (e.target === document.getElementById('purge-all-modal-overlay')) closePurgeAllModal();
}
function checkPurgeAllConfirmInput() {
    const val = document.getElementById('confirm-purge-all-input').value.trim().toUpperCase();
    const link = document.getElementById('modal-confirm-purge-all-link');
    if (val === 'DELETE') link.classList.remove('disabled'); else link.classList.add('disabled');
}

// ── Restore All Modal ──
function openRestoreAllModal() {
    document.getElementById('modal-confirm-restore-all-link').href = 'profit.php?action=restore_all&year=<?= $view_year ?>';
    document.getElementById('restore-all-modal-overlay').classList.add('open');
    document.body.style.overflow = 'hidden';
}
function closeRestoreAllModal() {
    document.getElementById('restore-all-modal-overlay').classList.remove('open');
    document.body.style.overflow = '';
}
function handleRestoreAllOverlayClick(e) {
    if (e.target === document.getElementById('restore-all-modal-overlay')) closeRestoreAllModal();
}

// Close on Escape key
// ── Delete Permanently (single) Modal ──
function openPurgeModal(id, productName, saleDate) {
    document.getElementById('purge-modal-sale-name').textContent = productName;
    document.getElementById('purge-modal-sale-date').textContent = 'Date: ' + saleDate;
    document.getElementById('purge-modal-confirm-link').href = 'profit.php?action=purge&id=' + id + '&year=<?= $view_year ?>';
...
function handlePurgeOverlayClick(e) {
    if (e.target === document.getElementById('purge-modal-overlay')) closePurgeModal();
}

document.addEventListener('keydown', function(e) {
    if (e.key === 'Escape') {
        closeDeleteModal(); closeDeleteAllModal();
        closeDeletedHistoryModal(); closePurgeModal(); closePurgeAllModal(); closeRestoreAllModal();
    }
});

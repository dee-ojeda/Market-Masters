...
function expiryBadge($expiry_date) {
    ...
    if ($diff < 0) {
        return ['label' => '⚠ Expired', 'class' => 'badge-expired', 'date' => $formatted];
    } elseif ($diff <= 30) {
        return ['label' => '⏳ ' . $diff . 'd left', 'class' => 'badge-orange', 'date' => $formatted];
    } else {
        return ['label' => $formatted, 'class' => 'badge-fresh', 'date' => $formatted];
    }
}
?>
<!DOCTYPE html>
                            ...
                            <tr class="prod-row"
                                ...
                                <td>
                                    <?php if (!empty($badge['class'])): ?>
                                        ...
                                        <?php if ($badge['class'] !== 'badge-fresh' && !empty($badge['date'])): ?>
